...
pub type ParseStatus<T> = result::Result<T, ParseFailed>;
...
pub fn eot() -> Parser<()> {
    ...
    if input.text[input.index] == EOT {
        result::Ok(Succeeded{new_state:
                                 State{index: input.index + 1u, ..input},
                             value: (),})
    } else {
        result::Err(Failed{old_state: input,
                           err_state: input,
                           mesg: @~"EOT",})
    }
}
...
pub fn anycp(predicate: @fn(char) -> bool) -> Parser<char> {
    ...
    if input.text[i] != EOT && predicate(input.text[i]) { i += 1u; }
    if i > input.index {
        result::Ok(Succeeded{new_state: State{index: i, ..input},
                             value: input.text[input.index],})
    } else {
        result::Err(Failed{old_state: input,
                           err_state: State{index: i, ..input},
                           mesg: @~"",})
    }
}
}
...
impl<'self> CharParsers for &'self str {
    fn anyc(&self) -> Parser<char> {
        ...
        |input: State| {
        let mut i = input.index;
        if str::find_char(s, input.text[i]).is_some() { i += 1u; }
        if i > input.index {
            result::Ok(Succeeded{new_state: State{index: i, ..input},
                                 value: input.text[input.index],})
        } else {
...
pub fn match0(predicate: @fn(char) -> bool) -> Parser<@~str> {
    |input: State| {
    let mut i = input.index;
    while input.text[i] != EOT && predicate(input.text[i]) { i += 1u; }
...
pub fn match1_0(prefix: @fn(char) -> bool, suffix: @fn(char) -> bool) ->
 Parser<@~str> {
...
pub fn optional_str(parser: Parser<@~str>) -> Parser<@~str> {
    ...
    match parser(input) {
        result::Ok(ref pass) =>
        result::Ok(Succeeded{new_state: pass.new_state, value: pass.value,}),
        result::Err(ref _failure) =>
        result::Ok(Succeeded{new_state: input, value: @~"",})
    }
}
}
...
pub fn scan(fun: @fn(@[char], uint) -> uint) -> Parser<@~str> {
    ...
    if count > 0u && input.text[i] != EOT
        // EOT check makes it easier to write funs that do stuff like matching chars that are not something
       {
        for uint::range(0u, count) |_k| {
            if input.text[i] == '\r' {
                line += 1;
        ...
        let text = str::from_chars(vec::slice(input.text, input.index, i));
        result::Ok(Succeeded{new_state: State{index: i, line: line, ..input},
                             value: @text,})
    } else {
        result::Ok(Succeeded{new_state: State{index: i, line: line, ..input},
                             value: @~"",})
...
pub fn seq2_ret_str<T0: Copy + Durable, T1: Copy +
                    Durable>(p0: Parser<T0>, p1: Parser<T1>) ->
 Parser<@~str> {
    |input: State| {
    match p0.then(p1)(input) {
        result::Ok(ref pass) => {
            let text =
                str::from_chars(vec::slice(input.text, input.index,
                                           pass.new_state.index));
...
/// If all the parsers are successful then the matched text is returned.
pub fn seq3_ret_str<T0: Copy + Durable, T1: Copy + Durable, T2: Copy +
                    Durable>(p0: Parser<T0>, p1: Parser<T1>, p2: Parser<T2>)
 -> Parser<@~str> {
    |input: State| {
...
/// If all the parsers are successful then the matched text is returned.
pub fn seq4_ret_str<T0: Copy + Durable, T1: Copy + Durable, T2: Copy +
                    Durable, T3: Copy +
                    Durable>(p0: Parser<T0>, p1: Parser<T1>, p2: Parser<T2>,
                             p3: Parser<T3>) -> Parser<@~str> {
    |input: State| {
...
/// If all the parsers are successful then the matched text is returned.
pub fn seq5_ret_str<T0: Copy + Durable, T1: Copy + Durable, T2: Copy +
                    Durable, T3: Copy + Durable, T4: Copy +
                    Durable>(p0: Parser<T0>, p1: Parser<T1>, p2: Parser<T2>,
                             p3: Parser<T3>, p4: Parser<T4>) ->
 Parser<@~str> {
    |input: State| {
...
impl<'self> StringParsers for &'self str {
    fn lit(&self) -> Parser<@~str> {
        let s = self.to_owned();
        |input: State| {
        let mut i = 0u;
        ...
        while i < str::len(s) {
            ...
            if ch == input.text[j] { i = next; j += 1u; } else { break ; }
        }
        if i == str::len(s) {
            let text =
                str::from_chars(vec::slice(input.text, input.index, j));
            result::Ok(Succeeded{new_state: State{index: j, ..input},
                                 value: @text,})
        } else {
            result::Err(Failed{old_state: input,
                               err_state: State{index: j, ..input},
                               mesg: @fmt!("\'%s\'" , s),})
        }
    }
    }
    ...
    fn s0(&self) -> Parser<@~str> { self.lit().s0() }
    fn s1(&self) -> Parser<@~str> { self.lit().s1() }
}
...
pub fn or_v<T: Copy + Durable>(parsers: @~[Parser<T>]) -> Parser<T> {
    ...
    let mut i = 0u;
    while i < vec::len(*parsers) && result.is_none() {
    ...
    if result.is_some() {
        ...
        result::Err(Failed{old_state: input,
                           err_state: State{index: max_index, ..input},
                           mesg: @mesg,})
    }
}
}

 // TODO: probably need to use &T instead
...
/// seq2 := e0 e1
pub fn seq2<T0: Copy + Durable, T1: Copy + Durable, R: Copy +
            Durable>(parser0: Parser<T0>, parser1: Parser<T1>,
                     eval: @fn(T0, T1) -> result::Result<R, @~str>) ->
 Parser<R> {
    do parser0.thene |a0| {
        do parser1.thene |a1| {
            match eval(a0, a1) {
                result::Ok(ref value) => { ret(*value) }
                result::Err(mesg) => { fails(*mesg) }
            }
        }
    }
}
...
/// seq3 := e0 e1 e2
pub fn seq3<T0: Copy + Durable, T1: Copy + Durable, T2: Copy + Durable,
            R: Copy +
            Durable>(parser0: Parser<T0>, parser1: Parser<T1>,
                     parser2: Parser<T2>,
                     eval: @fn(T0, T1, T2) -> result::Result<R, @~str>) ->
 Parser<R> {
    do parser0.thene |a0| {
        do parser1.thene |a1| {
            do parser2.thene |a2| {
...
/// seq4 := e0 e1 e2 e3
pub fn seq4<T0: Copy + Durable, T1: Copy + Durable, T2: Copy + Durable,
            T3: Copy + Durable, R: Copy +
            Durable>(parser0: Parser<T0>, parser1: Parser<T1>,
                     parser2: Parser<T2>, parser3: Parser<T3>,
                     eval: @fn(T0, T1, T2, T3) -> result::Result<R, @~str>) ->
 Parser<R> {
    do parser0.thene |a0| {
        do parser1.thene |a1| {
            do parser2.thene |a2| {
...
/// seq5 := e0 e1 e2 e3 e4
pub fn seq5<T0: Copy + Durable, T1: Copy + Durable, T2: Copy + Durable,
            T3: Copy + Durable, T4: Copy + Durable, R: Copy +
            Durable>(parser0: Parser<T0>, parser1: Parser<T1>,
                     parser2: Parser<T2>, parser3: Parser<T3>,
                     parser4: Parser<T4>,
                     eval:
                         @fn(T0, T1, T2, T3, T4) -> result::Result<R, @~str>)
 -> Parser<R> {
    do parser0.thene |a0| {
        do parser1.thene |a1| {
            do parser2.thene |a2| {
...
pub fn seq6<T0: Copy + Durable, T1: Copy + Durable, T2: Copy + Durable,
            T3: Copy + Durable, T4: Copy + Durable, T5: Copy + Durable,
            R: Copy +
            Durable>(parser0: Parser<T0>, parser1: Parser<T1>,
                     parser2: Parser<T2>, parser3: Parser<T3>,
                     parser4: Parser<T4>, parser5: Parser<T5>,
                     eval:
                         @fn(T0, T1, T2, T3, T4, T5)
                             -> result::Result<R, @~str>) -> Parser<R> {
    do parser0.thene |a0| {
        do parser1.thene |a1| {
            do parser2.thene |a2| {
                do parser3.thene |a3| {
...
pub fn seq7<T0: Copy + Durable, T1: Copy + Durable, T2: Copy + Durable,
            T3: Copy + Durable, T4: Copy + Durable, T5: Copy + Durable,
            T6: Copy + Durable, R: Copy +
            Durable>(parser0: Parser<T0>, parser1: Parser<T1>,
                     parser2: Parser<T2>, parser3: Parser<T3>,
                     parser4: Parser<T4>, parser5: Parser<T5>,
                     parser6: Parser<T6>,
...
pub fn seq8<T0: Copy + Durable, T1: Copy + Durable, T2: Copy + Durable,
            T3: Copy + Durable, T4: Copy + Durable, T5: Copy + Durable,
            T6: Copy + Durable, T7: Copy + Durable, R: Copy +
            Durable>(parser0: Parser<T0>, parser1: Parser<T1>,
                     parser2: Parser<T2>, parser3: Parser<T3>,
                     parser4: Parser<T4>, parser5: Parser<T5>,
                     parser6: Parser<T6>, parser7: Parser<T7>,
                     eval:
...
pub fn seq9<T0: Copy + Durable, T1: Copy + Durable, T2: Copy + Durable,
            T3: Copy + Durable, T4: Copy + Durable, T5: Copy + Durable,
            T6: Copy + Durable, T7: Copy + Durable, T8: Copy + Durable,
            R: Copy +
            Durable>(parser0: Parser<T0>, parser1: Parser<T1>,
                     parser2: Parser<T2>, parser3: Parser<T3>,
                     parser4: Parser<T4>, parser5: Parser<T5>,
...
/// seq2_ret0 := e0 e1
pub fn seq2_ret0<T0: Copy + Durable, T1: Copy +
                 Durable>(p0: Parser<T0>, p1: Parser<T1>) -> Parser<T0> {
    seq2(p0, p1, |a0, _a1| result::Ok(a0))
}

/// seq2_ret1 := e0 e1
pub fn seq2_ret1<T0: Copy + Durable, T1: Copy +
                 Durable>(p0: Parser<T0>, p1: Parser<T1>) -> Parser<T1> {
    seq2(p0, p1, |_a0, a1| result::Ok(a1))
}

/// seq3_ret0 := e0 e1 e2
pub fn seq3_ret0<T0: Copy + Durable, T1: Copy + Durable, T2: Copy +
                 Durable>(p0: Parser<T0>, p1: Parser<T1>, p2: Parser<T2>) ->
 Parser<T0> {
...
/// seq3_ret1 := e0 e1 e2
pub fn seq3_ret1<T0: Copy + Durable, T1: Copy + Durable, T2: Copy +
                 Durable>(p0: Parser<T0>, p1: Parser<T1>, p2: Parser<T2>) ->
 Parser<T1> {
...
/// seq3_ret2 := e0 e1 e2
pub fn seq3_ret2<T0: Copy + Durable, T1: Copy + Durable, T2: Copy +
                 Durable>(p0: Parser<T0>, p1: Parser<T1>, p2: Parser<T2>) ->
 Parser<T2> {
...
pub fn seq4_ret0<T0: Copy + Durable, T1: Copy + Durable, T2: Copy + Durable,
                 T3: Copy +
                 Durable>(p0: Parser<T0>, p1: Parser<T1>, p2: Parser<T2>,
                          p3: Parser<T3>) -> Parser<T0> {
    seq4(p0, p1, p2, p3, |a0, _a1, _a2, _a3| result::Ok(a0))
}

/// seq4_ret1 := e0 e1 e2 e3
pub fn seq4_ret1<T0: Copy + Durable, T1: Copy + Durable, T2: Copy + Durable,
                 T3: Copy +
                 Durable>(p0: Parser<T0>, p1: Parser<T1>, p2: Parser<T2>,
                          p3: Parser<T3>) -> Parser<T1> {
    seq4(p0, p1, p2, p3, |_a0, a1, _a2, _a3| result::Ok(a1))
}

/// seq4_ret2 := e0 e1 e2 e3
pub fn seq4_ret2<T0: Copy + Durable, T1: Copy + Durable, T2: Copy + Durable,
                 T3: Copy +
                 Durable>(p0: Parser<T0>, p1: Parser<T1>, p2: Parser<T2>,
                          p3: Parser<T3>) -> Parser<T2> {
    seq4(p0, p1, p2, p3, |_a0, _a1, a2, _a3| result::Ok(a2))
}

/// seq4_ret3 := e0 e1 e2 e3
pub fn seq4_ret3<T0: Copy + Durable, T1: Copy + Durable, T2: Copy + Durable,
                 T3: Copy +
                 Durable>(p0: Parser<T0>, p1: Parser<T1>, p2: Parser<T2>,
                          p3: Parser<T3>) -> Parser<T3> {
    seq4(p0, p1, p2, p3, |_a0, _a1, _a2, a3| result::Ok(a3))
}

// chain_suffix := (op e)*
#[doc(hidden)]
pub fn chain_suffix<T: Copy + Durable, U: Copy +
                    Durable>(parser: Parser<T>, op: Parser<U>) ->
 Parser<@~[(U, T)]> {
    let q =
        op.thene(|operator| { parser.thene(|value| { ret((operator, value)) })
             });
    q.r0()
}
...
pub fn or_mesg(mesg1: @~str, mesg2: @~str) -> @~str {
    if !str::is_empty(*mesg1) && !str::is_empty(*mesg2) {
        ...
        mesg1
    } else if !str::is_empty(*mesg2) { mesg2 } else { @~"" }
}
...
pub trait Combinators<T: Copy + Durable> {
    ...
    fn chainl1<U: Copy + Durable>
        (&self, op: Parser<U>, eval: @fn(T, U, T) -> T) -> Parser<T>;
    /// chainr1 := e (op e)*
    /// 
    /// Right associative binary operator. eval is called for each parsed op.
    fn chainr1<U: Copy + Durable>
        (&self, op: Parser<U>, eval: @fn(T, U, T) -> T) -> Parser<T>;
    ...
    fn thene<U: Copy + Durable>(&self, eval: @fn(T) -> Parser<U>)
        -> Parser<U>;
}

impl <T: Copy + Durable> Combinators<T> for Parser<T> { fn chainl1<U: Copy +
               Durable>(&self, op: Parser<U>, eval: @fn(T, U, T) -> T) -> Parser<T> {
        |input: State| {
        do result::chain((*self)(input)) |pass| {
            match chain_suffix(*self, op)(pass.new_state) {
                result::Ok(ref pass2) => {
                    let value =
                        vec::foldl(pass.value, *pass2.value,
                                   |lhs: T, rhs: &(U, T)| {
                                   eval(lhs, rhs.first(), rhs.second()) });
                    result::Ok(Succeeded{new_state: pass2.new_state,
                                         value: value,})
                }
                result::Err(ref failure) => {
                    result::Err(Failed{old_state: input, ..*failure})
            ...
            match chain_suffix(*self, op)(pass.new_state) {
                result::Ok(ref pass2) => {
                    if !vec::is_empty(*pass2.value) {
                        ...
                        let parsers =
                            ~[e1] +
                                vec::slice(parsers, 0u,
                                           vec::len(parsers) - 1u);
                        let terms = vec::zip(parsers, ops);
                        let cl = |lhs: &(T, U), rhs: &T| eval(lhs.first(), lhs.second(), *rhs);
                        let value = vec::foldr(terms, e3, cl);
    ...
    fn err(&self, label: &str) -> Parser<T> {
        ...
        // empty strings there).
        do result::chain_err((self.note(label))(input)) |failure| {
            if str::is_empty(label) {
                result::Err(Failed{mesg: @~"", ..failure})
            } else if failure.err_state.index == input.index ||
                          str::is_empty(*failure.mesg) {
                result::Err(Failed{mesg: @copy label, ..failure})
            } else { result::Err(failure) }
        }
    }
    }
    fn everything<U: Copy + Durable>(&self, space: Parser<U>) -> Parser<T> {
    ...
    fn note(&self, mesg: &str) -> Parser<T> {
        ...
        match (*self)(input) {
            result::Ok(ref pass) => {
                ...
                assert!(pass . new_state . index >= input . index);
                // can't go backwards on success (but no progress is fine, eg e*)
                if pass.new_state.index > input.index {
                    info!("%s" , munge_chars ( input . text ));
                    info!("%s^ %s parsed \'%s\'" , repeat_char
                          ( ' ' , pass . new_state . index ) , mesg , str ::
                          slice
                          ( munge_chars ( input . text ) , input . index ,
                          pass . new_state . index ));
                } else {
                    info!("%s" , munge_chars ( input . text ));
                    info!("%s^ %s passed" , repeat_char
                          ( ' ' , pass . new_state . index ) , mesg);
                }
                result::Ok(*pass)
            }
            result::Err(ref failure) => {
                assert!(failure . old_state . index == input . index);
                // on errors the next parser must begin at the start
                assert!(failure . err_state . index >= input . index);
                // errors can't be before the input
                debug!("%s" , munge_chars ( input . text ));
                if failure.err_state.index > input.index {
                    debug!("%s^%s! %s failed" , repeat_char
                           ( '-' , input . index ) , repeat_char
                           ( ' ' , failure . err_state . index - input . index
                           ) , mesg);
                } else {
                    debug!("%s^ %s failed" , repeat_char
                           ( '-' , input . index ) , mesg);
                }
                result::Err(*failure)
            }
    ...
    fn optional(&self) -> Parser<Option<T>> {
        ...
        match (*self)(input) {
            result::Ok(ref pass) => {
                result::Ok(Succeeded{new_state: pass.new_state,
                                     value: option::Some(pass.value),})
            }
            result::Err(ref _failure) => {
    ...
    fn or(&self, parser2: Parser<T>) -> Parser<T> {
        |input: State| {
        do result::chain_err((*self)(input)) |failure1| {
            do result::chain_err(parser2(input)) |failure2| {
                if failure1.err_state.index > failure2.err_state.index {
                ...
                } else if failure1.err_state.index < failure2.err_state.index
                 {
                    ...
                    result::Err(Failed{mesg:
                                           or_mesg(failure1.mesg,
                                                   failure2.mesg),
                                                                     ..failure2})
                }
            }
    ...
    fn parse(&self, file: @~str, text: &str) -> ParseStatus<T> {
        ...
        match (*self)(input) {
            result::Ok(ref pass) => { result::Ok(pass.value) }
            result::Err(ref failure) => {
                let col = get_col(chars, failure.err_state.index);
                result::Err(ParseFailed{file: failure.old_state.file,
                                        line: failure.err_state.line as uint,
                                        col: col,
                                        mesg: failure.mesg,})
            }
        }
    ...
    fn r(&self, n: uint, m: uint) -> Parser<@~[T]> {
        ...
        loop  {
            match (*self)(output) {
                result::Ok(ref pass) => {
                    assert!(pass . new_state . index > output .
                            index); // must make progress to ensure loop terminatio);
    ...
    fn r0(&self) -> Parser<@~[T]> { self.r(0u, uint::max_value) }
    fn r1(&self) -> Parser<@~[T]> { self.r(1u, uint::max_value) }
    fn s0(&self) -> Parser<T> {
        |
    ...
    fn s1(&self) -> Parser<T> {
        |input: State| {
         // little cheesy, but saves us from adding a helper fn
        do result::chain(self.s0()(input)) |pass| {
            if str::find_char(" \t\r\n", input.text[pass.new_state.index - 1u]).is_some() {
                result::Ok(pass)
            } else {
                result::Err(Failed{old_state: input,
                                   err_state: pass.new_state,
                                   mesg: @~"whitespace",})
            }
        }
    }
    ...
    fn then<U: Copy + Durable>(&self, parser2: Parser<U>) -> Parser<U> {
        |input: State| {
        do result::chain((*self)(input)) |pass| {
            do result::chain_err(parser2(pass.new_state)) |failure| {
                result::Err(Failed{old_state: input, ..failure})
            }
        }
    }
    ...
    fn thene<U: Copy + Durable>(&self, eval: @fn(T) -> Parser<U>) ->
     Parser<U> {
        |input: State| {
        do result::chain((*self)(input)) |pass| {
            do result::chain_err(eval(pass.value)(pass.new_state)) |failure| {
                result::Err(Failed{old_state: input, ..failure})
            }
        }
    }
...
impl <'self> GenericParsers for &'self str {
    fn litv<T: Copy + Durable>(&self, value: T) -> Parser<T> {
        let s = self.to_owned();
        |input: State| {
        match s.lit()(input) {
            ...
            result::Err(ref failure) => { result::Err(*failure) }
        }
    }
    }
}
