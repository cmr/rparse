...
pub fn decimal_number() -> Parser<int> {
    do match1(is_digit).thene |text| {
        match int::from_str(*text) {
            option::Some(value) => { ret(value) }
            _ => { fails(fmt!("\'%s\' is out of range" , * text)) }
        }
    }
}
...
pub fn octal_number() -> Parser<int> {
    do match1_0(|c| c == '0', is_octal).thene |text| {
        match from_base_8(*text) {
            result::Ok(value) => { ret(value) }
            result::Err(copy mesg) => { fails(mesg) }
        }
    }
}
...
pub fn float_number() -> Parser<f64> {
    let exponent =
        seq3_ret_str("eE".anyc(), "+-".anyc().optional(), match1(is_digit));
    let float1 =
        seq4_ret_str(match0(is_digit), ".".lit(), match1(is_digit),
                     exponent.optional()).err("");
    let float2 =
        seq3_ret_str(match1(is_digit), ".".lit(),
                     exponent.optional()).err("");
    let float3 = seq2_ret_str(match1(is_digit), exponent).err("");
    let number = or_v(@~[float1, float2, float3]);
    do number.thene |text| {
...
// ---- Helpers ---------------------------------------------------------------
fn is_identifier_prefix(ch: char) -> bool {
    return is_alphabetic(ch) || ch == '_';
}

fn is_identifier_suffix(ch: char) -> bool {
    return is_identifier_prefix(ch) || is_digit(ch);
}

fn is_octal(ch: char) -> bool { return ch >= '0' && ch <= '7'; }
...
fn from_base_8(text: &str) -> result::Result<int, ~str> {
    ...
    while i > 0u {
        ...
        if power * delta < int::max_value - result {
            result += power * delta;
        } else { return result::Err(~"Octal number is too large"); }
        power *= 8;
    }
...
fn from_base_16(text: &str) -> result::Result<int, ~str> {
    ...
    while i > 0u {
        ...
        let delta =
            ...
            } else { 10 + (text[i] - ('A' as u8)) as int };
        if power * delta < int::max_value - result {
            result += power * delta;
        } else { return result::Err(~"Hex number is too large"); }
        power *= 16;
    }
    return result::Ok(result);
}
...
fn escape_sequence() -> Parser<char> {
    let escape =
        do "'\"?abfnrtv\\".anyc().thene |ch| { ret(escape_to_char(ch)) };
    let case1 = seq2_ret1("\\".lit(), escape);
    let case2 =
        seq2_ret1("\\".lit(), octal_digits().thene(|n| ret(n as char)));
    let case3 =
        seq2_ret1("\\x".lit(), hex_digits().thene(|n| ret(n as char)));
